...
并排半环（运动手表那种），≤4。语法与 progress 相同：`名称, 百分比` 或 `名称, 数值, 目标`。数字在环内，名称在环下。多指标对照用这个；叠在一起的达成率用 progress。
...
同一指标的两列数字（优化前/后、我们/他们），≤6 行。可选表头 `, 左列名, 右列名`。定性方案对比用 compare；多类柱状用 chart bar。
...
按周排列的日历热力（办公卡，不是 GitHub 小绿点）。`YYYY-MM-DD, 次数`，缺天当 0，周从周一起，跨度 ≤12 周。格子随周数放大，有值的格子写次数。全年或小时级交给 AntV。
